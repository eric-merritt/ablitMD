...
PCA_RANGE = (
    38,
    63,
)  # layer range averaged for the PCA, matching the prior direction_pca.json
...
def main():
    ...
    for step in run["sequence"]:
        ...
        print(
            f"[recompute] {model_id}/{mode}: {len(per_category)} categories with a direction"
        )

        if not pca_written:
            count = write_pca(
                per_category, _root / "frontend" / "public" / "direction_pca.json"
            )
    ...
    print(
        f"[recompute] wrote {sidecar.name}: {sidecar.stat().st_size / 1048576:.2f} MB"
    )
