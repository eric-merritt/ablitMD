...
interface VerifyContext { modelId: string; apiModelId: string; genMode: string; samplesPerCategory: number; mode: AblationMode; classicFactor: number; disclaimerAblate: boolean; disclaimerFactor: number }
...
const App = () => {
  ...
  const handleVerifyStart = (modelId: string, apiModelId: string, genMode: string, mode: AblationMode, classicFactor: number, disclaimerAblate: boolean, disclaimerFactor: number) => {
    setVerifyContext({ modelId, apiModelId, genMode, samplesPerCategory: 2, mode, classicFactor, disclaimerAblate, disclaimerFactor })
    setPhase('verify')
  }
    ...
    <>
      ...
      { phase === 'verify' && activeRun && verifyContext && (
        <VerifyDashboard
          ...
          apiModelId={ verifyContext.apiModelId }
