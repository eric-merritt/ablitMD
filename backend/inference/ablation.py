...
def ablate_hidden(
    hidden: torch.Tensor, directions: list[tuple[torch.Tensor, float]]
) -> torch.Tensor:
...
def orthogonalize_weight(
    weight: torch.Tensor, direction: torch.Tensor, factor: float
) -> torch.Tensor:
    weight.addr_(direction, direction @ weight, alpha=-factor)
    return weight


def orthogonalize_input(
    weight: torch.Tensor, direction: torch.Tensor, factor: float
) -> torch.Tensor:
...
def compute_classic_directions(
    ...
    if not refused_states or not complied_states:
        raise ValueError(
            "need both refused and complied hidden states to compute classic directions"
        )
    ...
    for layer_idx in range(n_layers):
        diff = refused_arr[:, layer_idx, :].mean(axis=0) - complied_arr[
            :, layer_idx, :
        ].mean(axis=0)
    ...
    if include_disclaimer and disclaimer_states:
        ...
        for layer_idx in range(n_layers):
            diff = disc_arr[:, layer_idx, :].mean(axis=0) - complied_arr[
                :, layer_idx, :
            ].mean(axis=0)
            norm = float(np.linalg.norm(diff))
            if norm < 1e-8:
...
def apply_classic_in_place(
    ...
    def _snap_and_edit(proj, direction_t):
      if id(proj) not in snapshots:
        snapshots[id(proj)] = (proj, _get_weight_f32(proj).clone())
      W = _get_weight_f32(proj)
      _set_weight(proj, orthogonalize_weight(W, direction_t.to(torch.float32), factor), dtype)
    ...
    if disclaimer_directions:
        for layer_idx, d_disc in disclaimer_directions.items():
            ...
            if d_ref is not None:
                ...
                if res_norm > 1e-4:
                    effective_disclaimer[layer_idx] = (d_res / res_norm).astype(
                        np.float32
                    )
            else:
    ...
    with torch.no_grad():
        for layer_idx, direction in directions.items():
            ...
            if layer_idx == 0:
                ...
                emb.weight.data.addr_(
                    direction_t @ emb.weight.data.T, direction_t, alpha=-factor
                )
                continue
        ...
        if effective_disclaimer:
            ...
            def _snap_and_edit_disc(proj, direction_t):
              if id(proj) not in snapshots:
                snapshots[id(proj)] = (proj, _get_weight_f32(proj).clone())
              W = _get_weight_f32(proj)
              _set_weight(proj, orthogonalize_weight(W, direction_t.to(torch.float32), disclaimer_factor), dtype)
            ...
            for layer_idx, direction in effective_disclaimer.items():
                ...
                if layer_idx == 0:
                    ...
                    if id(emb) not in snapshots:
                        snapshots[id(emb)] = (emb, emb.weight.data.clone())
                    emb.weight.data.addr_(
                        direction_t.to(torch.float32)
                        @ emb.weight.data.to(torch.float32).T,
                        direction_t.to(torch.float32),
                        alpha=-disclaimer_factor,
                    )
                    continue
                decoder_idx = layer_idx - 1
                if decoder_idx >= len(layers):
        ...
        if lm_head is not None and hasattr(lm_head, "weight"):
            ...
            _set_weight(lm_head, W.to(device), dtype)
...
def compare_directions(
    ...
    for layer_idx in all_layers:
        ...
        if classic_vec is not None:
            ...
            classic_unit = (
                classic_vec / classic_norm if classic_norm > 1e-8 else classic_vec
            )
        else:
            classic_unit = None
...
def directions_match_hook(
    baked: torch.Tensor, hooked: torch.Tensor, atol: float = 1e-5
) -> bool:
