...
def _check_model_clean(model, sample_layers: list[int] = [10, 20, 30, 40, 50]) -> None:
    ...
    if not LAYER_HASH_PATH.exists():
        print(
            "[bake] WARNING: data/layerHash.json not found — cannot verify model is clean before bake",
            flush=True,
        )
        return
    ...
    if dirty:
        print(
            "[bake] WARNING: model weights were already modified before bake — output will be double-ablated",
            flush=True,
        )
