...
def _check_model_clean(model, sample_layers: list[int] = [10, 20, 30, 40, 50]) -> None:
  """Log whether in-memory model weights match the reference hashes.
  Only runs if data/layerHash.json exists (generated by scripts/hash_reference_model.py)."""
  import hashlib
  if not LAYER_HASH_PATH.exists():
    print("[bake] WARNING: data/layerHash.json not found — cannot verify model is clean before bake", flush=True)
    return
  ...
  for name, param in model.named_parameters():
    if not any(name.endswith(s) for s in PROJ_SUFFIXES):
      continue
    layer_match = next((p for p in name.split(".") if p.isdigit()), None)
    if layer_match is None or int(layer_match) not in sample_layers:
      continue
    if name not in ref:
      missing.append(name)
      continue
    current_hash = hashlib.sha256(param.data.cpu().contiguous().to(torch.bfloat16).view(torch.uint8).numpy().tobytes()).hexdigest()
    (clean if current_hash == ref[name] else dirty).append(name)
  print(
    f"[bake] pre-bake weight check: {len(clean)} clean, {len(dirty)} dirty, {len(missing)} not in reference",
    flush=True
  )
  for name in dirty:
    print(f"[bake]   DIRTY: {name}", flush=True)
  if dirty:
    print("[bake] WARNING: model weights were already modified before bake — output will be double-ablated", flush=True)
